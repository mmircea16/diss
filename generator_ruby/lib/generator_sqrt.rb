...
class GeneratorSqrt < Generator
 ...
 def generate_test test_no
     ...
     case @operand_type
     when "8_8":
       rez = @util.float_to_signed(Math.sqrt(n),8,8)
     when "16_16":
       rez = @util.float_to_signed(Math.sqrt(n),16,16)
     when "8_24":
       rez = @util.float_to_signed(Math.sqrt(n),8,24);
     when "24_8":
       rez = @util.float_to_signed(Math.sqrt(n),24,8);
     end
